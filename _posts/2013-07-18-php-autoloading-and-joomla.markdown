...
This is where Joomla comes in. Joomla didn't choose to follow that widely accepted convention, but instead offered its own solution. For about a year and a half, Joomla has had a prefix based autoloader. Instead of using underscores to represent a directory change, the prefix autoloader uses [StudlyCaps](http://en.wikipedia.org/wiki/StudlyCaps). When parsing the class name into a file location, the autoloader first strips the prefix from the class name then inserts a directory separator right before each capital letter. (One caveat to this is that if your class name is only two sections long, Joomla will duplicate the last section when loading from the filesystem. So class `FooBar` becomes `foo/bar/bar.php`.) It then lowercases the whole path and appends the `.php` file suffix to generated path. Finally, it loops through all paths registered to the matched prefix until it finds the file.
